...
describe('ChainLinkExample', async function () {
    let dai: TokenMock, daiWallet: TokenMock, oracle: AggregatorMock, apyOracle: OracleMock, swap: LimitOrderProtocol;
    ...
    function buildSinglePriceGetter (oracle: AggregatorMock, inverse: boolean, spread: string, amount = '0') {
        let abi = [
    ...
    function buildOrder (
        ...
        if (isFixedTaker) {
            makerGetter = cutLastArg(buildSinglePriceGetter(oracle, false, '1010000000'));
            takerGetter = cutLastArg(buildSinglePriceGetter(oracle, true, '990000000'));
        } else {
            makerGetter = cutLastArg(buildSinglePriceGetter(oracle, true, '1010000000'));
            takerGetter = cutLastArg(buildSinglePriceGetter(oracle, false, '990000000'));
        } 
        return {
            salt: salt,
            asset: asset,
    ...
    beforeEach(async function () {
        ...
        oracle = (await AggregatorMockFactory.deploy("737869762948382064")) as AggregatorMock;
    ...
    it('chainlink+eps order', async function () {
        ...
        const order = buildOrder(
            "1", dai.address, dai.address, "5000000", "1000000",
            true,
        ...
        const response = await swap.fillOrder(order, signature, "5000000", 0, "999900");
        ...
        expect((await swap.orderParticipantFixedTokens(orderHash, wallet)).toNumber()).to.be.equal(4039999);
        expect((await swap.orderParticipantVariableTokens(orderHash, wallet)).toNumber()).to.be.equal(1000000);
        expect((await swap.orderParticipantFixedTokens(orderHash, _)).toNumber()).to.be.equal(4039999);
        expect((await swap.orderParticipantVariableTokens(orderHash, _)).toNumber()).to.be.equal(1000000);
    });
    ...
    it('stop loss order', async function () {
        ...
        const endTimestamp = startTimestamp + 31536000;
        const order = buildOrder(
            "1", dai.address, dai.address, "5000000", "2000000",
            true,
            startTimestamp.toString(),
            endTimestamp.toString(),
            toBN("18446744073709551616").toString()
        ...
        expect((await swap.orderParticipantFixedTokens(orderHash, wallet)).toNumber()).to.be.equal(5000000);
        expect((await swap.orderParticipantVariableTokens(orderHash, wallet)).toNumber()).to.be.equal(1237500);
        expect((await swap.orderParticipantFixedTokens(orderHash, _)).toNumber()).to.be.equal(5000000);
        expect((await swap.orderParticipantVariableTokens(orderHash, _)).toNumber()).to.be.equal(1237500);
    });
});
