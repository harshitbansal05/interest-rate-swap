...
describe("LimitOrderProtocol", async function () {
    ...
    function makerTokensGetter(
        ...
        return iface.encodeFunctionData("getMakerAmount", [
            orderMakerAmount,
            orderTakerAmount,
            amount,
        ]);
    ...
    function takerTokensGetter(
        ...
        const iface = new ethers.utils.Interface(abi);
        return iface.encodeFunctionData("getTakerAmount", [
            orderMakerAmount,
            orderTakerAmount,
            amount,
        ]);
    ...
    function buildOrder(
        ...
        return {
            ...
            getMakerAmount: cutLastArg(
                makerTokensGetter(makerAmount, takerAmount)
            ),
            getTakerAmount: cutLastArg(
                takerTokensGetter(makerAmount, takerAmount)
            ),
            predicate,
            permit,
            interaction,
    ...
    describe("wip", async function () {
        ...
        it("should not swap with bad signature", async function () {
            ...
            await expect(
                swap.fillOrder(sentOrder, signature, 1, 0, 1)
            ).to.be.revertedWith("LOP: bad signature");
        });
        ...
        it("should not fill (1,1)", async function () {
            ...
            await daiWallet.approve(swap.address, "1000000");
            await expect(
                swap.fillOrder(order, signature, 1, 1, 1)
            ).to.be.revertedWith("LOP: only one amount should be 0");
        });
        ...
        it("should not fill above threshold", async function () {
            ...
            await daiWallet.approve(swap.address, "1000000");
            await expect(
                swap.fillOrder(order, signature, 1, 0, 2)
            ).to.be.revertedWith("LOP: Tokens less than threshold");
        });
        ...
        it("should not fill below threshold", async function () {
            ...
            await daiWallet.approve(swap.address, "1000000");
            await expect(
                swap.fillOrder(order, signature, 0, 2, 1)
            ).to.be.revertedWith("LOP: Tokens less than threshold");
        });
        ...
        it("should fail when both amounts are zero", async function () {
            ...
            await daiWallet.approve(swap.address, "1000000");
            await expect(
                swap.fillOrder(order, signature, 0, 0, 0)
            ).to.be.revertedWith("LOP: only one amount should be 0");
        });
        ...
        it("should swap fully based on signature", async function () {
            ...
            expect(
                (
                    await swap.orderParticipantFixedTokens(orderHash, _)
                ).toNumber()
            ).to.be.equal(1);
            expect(
                (
                    await swap.orderParticipantVariableTokens(orderHash, _)
                ).toNumber()
            ).to.be.equal(1);

            expect((await dai.balanceOf(wallet)).toNumber()).to.be.lessThan(
                makerDai.toNumber()
            );
            expect((await dai.balanceOf(_)).toNumber()).to.be.lessThan(
                takerDai.toNumber()
            );
            expect(
                (await dai.balanceOf(swap.address)).toNumber()
            ).to.be.greaterThan(swapDai.toNumber());
        });
        ...
        it("should fail on floor maker amount = 0", async function () {
            ...
            await daiWallet.approve(swap.address, "1000000");
            await expect(
                swap.fillOrder(order, signature, 4, 0, 0)
            ).to.be.revertedWith("LOP: can't swap 0 amount");
        });
    ...
    describe("Permit", function () {
        it("fillOrderToWithPermit", async function () {
            const orderMakerPermit = await getPermit(
                wallet,
                walletPrivateKey,
                dai,
                "1",
            ...
            const targetorderMakerPermitPair = withTarget(
                dai.address,
                orderMakerPermit
            );
            const orderTakerPermit = await getPermit(
                _,
                _PrivateKey,
                dai,
                "1",
            ...
            const targetorderTakerPermitPair = withTarget(
                dai.address,
                orderTakerPermit
            );
            ...
            const takerDai = await dai.balanceOf(_);
            const swapDai = await dai.balanceOf(swap.address);
            const response = await swap.fillOrderToWithPermit(
                order,
                signature,
                1,
                0,
                1,
                targetorderTakerPermitPair
            );
            const receipt = await response.wait(1);
    ...
    describe("Amount Calculator", async function () {
        ...
        it("empty getTakerAmount should not work on partial fill", async function () {
            ...
            await daiWallet.approve(swap.address, "1000000");
            await expect(
                swap.fillOrder(order, signature, 1, 0, 1)
            ).to.be.revertedWith("LOP: wrong amount");
        });
        ...
        it("empty getMakerAmount should not work on partial fill", async function () {
            ...
            await daiWallet.approve(swap.address, "1000000");
            await expect(
                swap.fillOrder(order, signature, 0, 1, 1)
            ).to.be.revertedWith("LOP: wrong amount");
        });
    });
    ...
    describe("Order Cancelation", async function () {
        ...
        it("should not cancel foreign order", async function () {
            await expect(swap.cancelOrder(order)).to.be.revertedWith(
                "LOP: Access denied"
            );
        });
    });
    ...
    describe("Private Orders", async function () {
        ...
        it("should not fill with incorrect taker", async function () {
            ...
            await expect(
                swap.fillOrder(order, signature, 1, 0, 1)
            ).to.be.revertedWith("LOP: private order");
        });
    });
    ...
    describe("Predicate", async function () {
        ...
        beforeEach(async function () {
            ...
            const abiToken = ["function balanceOf(address account)"];
        ...
        it("`or` should pass", async function () {
            ...
            const balanceCall = ifaceToken.encodeFunctionData("balanceOf", [
                wallet,
            ]);
            const gt = iface.encodeFunctionData("gt", [
                "100",
                dai.address,
                balanceCall,
            ]);
            const predicate = iface.encodeFunctionData("or", [
        ...
        it("`or` should fail", async function () {
            ...
            const lt = iface.encodeFunctionData("lt", [
                "100",
                dai.address,
                balanceCall,
            ]);
        ...
        it("`and` should pass", async function () {
            ...
            const predicate = iface.encodeFunctionData("and", [
                [swap.address, swap.address],
                [ts, gt],
            ]);
            const order = buildOrder(
        ...
        it("nonce + ts example", async function () {
            ...
            const endTimestamp = startTimestamp + 31536000;
            const ts = iface.encodeFunctionData("timestampBelow", [0xff000000]);
            const nonceCall = iface.encodeFunctionData("nonceEquals", [
                wallet,
                0,
            ]);
            const predicate = iface.encodeFunctionData("and", [
